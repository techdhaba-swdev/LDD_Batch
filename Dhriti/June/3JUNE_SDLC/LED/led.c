...
static struct cdev led_cdev; //CD Structure
...
static int led_state = 0; // 0 for off, 1 for on
...
static ssize_t led_read(struct file *filp, char __user *user_buf, size_t count, loff_t *f_pos) {
    char value = led_state + '0'; // Convert  to character 0/1
    ...
    return sizeof(value); // Return the number of bytes read
}
