...
#define DEVICE_NAME "mmep_buffer"
...
static ssize_t my_device_write(struct file *file, const char __user *buffer, size_t length, loff_t *offset) {
    int num1, num2, sum;
    if (copy_from_user(device_memory, buffer, length)) {
        return -EFAULT;
    }

    sscanf(device_memory, "%d %d", &num1, &num2);
    sum = num1 + num2;
...
MODULE_DESCRIPTION("Example Device Driver with addition operation");
