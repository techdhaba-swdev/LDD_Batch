...
static ssize_t device_write(struct file *filp, const char __user *buffer, size_t length, loff_t *offset) {
    ...
    if (written > 0) {
	    if(copy_from_user(user_input,buffer,length)){
		    return -EFAULT;
	    }
	    user_input[length] = '\0';
	    if(sscanf(user_input,"%d %d",&num1,&num2) == 2)
	    {
		    result = num1+num2;
		    snprintf(kernel_buffer,BUF_SIZE,"%d",result);
		    printk(KERN_INFO "Kernel computed result: %d\n", result);
	    }else{
		    printk(KERN_WARNING "Kernel receive invalid input\n");
		    return -EFAULT;
	    }
    }
    return written;
