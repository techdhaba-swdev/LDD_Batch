...
int main() {
    ...
    const char *values[] = {"10+5", "10-5", "20+5", "20-5", "30+5", "30-5", "40+5", "40-5", "50+5", "50-5"};
   
    for (int i = 0; i < 10; i++) {
        // Writing arithmetic expression to the device
        size_t len = strlen(values[i]) + 1; // +1 to include null terminator
        memcpy(mapped_mem, values[i], len);
        printf("User wrote: %s\n", values[i]);

        // Reading result from the device
        printf("User read result: %d\n", strtol(mapped_mem, NULL, 10));
    }
