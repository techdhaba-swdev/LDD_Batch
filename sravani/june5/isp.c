Interface Segregation Principle (ISP) : states that a client should not be exposed to methods it doesn’t need.

example :

To adhere to ISP, we can separate the device operations (open, release, ioctl) into distinct functions, each handling a single responsibility.
