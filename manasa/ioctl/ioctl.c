...
static char message[100] = "hi manasa"; // Stores the received string
...
// IOCTL operation
static long reverse_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
  switch (cmd) {
    case REVERSE_STRING:
      reverse_string(message, message_len); // Reverse the string
      printk(KERN_INFO "Received and reversed string: %s\n", message);
      break;
    default:
...
static const struct file_operations reverse_dev_fops = {
  ...
  .unlocked_ioctl = reverse_dev_ioctl,
