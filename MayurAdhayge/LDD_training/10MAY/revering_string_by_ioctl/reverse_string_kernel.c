...
#define DEVICE_NAME "string_reversing_char_driver"
...
static long device_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param) {
 ...
 int end;
    switch (ioctl_num) {
	    ...
	    while(start != end)
	    {
             msg[start]=msg[start]^msg[end];
	     msg[end]=msg[start]^msg[end];
	     msg[start]=msg[start]^msg[end];
	    start++;
	    end--;
	    }
    ...
    pr_info("string length = %d  %s",msg_len,temp);
    return 0;
}
...
MODULE_DESCRIPTION("A simple character device driver for reversing the string");
MODULE_VERSION("1.0");
