...
#define DEVICE_NAME "data_transfer_spin"
...
static ssize_t my_write(struct file *filp, const char __user *user_buf,
	...
	if (copy_to_user(user_buf, &data, count )) {
		...
		pr_err("error coping the data to user\n");
		return -EFAULT;
	}
	//Process data to  user space 
	pr_info("transfer data : value = %d, message= %s",data.value, data.message);
	spin_unlock_irqrestore(&my_lock, flags);
