...
void *consumer(void *arg) {     
 ...
 pthread_mutex_lock(&mutex);         // Wait if the buffer is empty
while (items == 0) {           
...
// Remove the item from the buffer      
  ...
  // Signal the producer that space is now available        
   pthread_cond_signal(&empty);         // Unlock the mutex       
...
int main(int argc, char *argv) {     
	...
	pthread_t producers[num_producers], consumers[num_consumers];     // Create 
