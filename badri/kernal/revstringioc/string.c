...
static long reverse_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    if (copy_to_user(user_buf, reversed_string, len + 1)) {
        return -EFAULT;
    }
...
static struct file_operations fops = {
    ...
    .write = reverse_write,
    .read = reverse_read,
    .unlocked_ioctl = reverse_ioctl, // Add ioctl handler
};
...
MODULE_DESCRIPTION("Kernel module for string reversal");
