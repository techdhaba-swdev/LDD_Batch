...
static ssize_t led_read(struct file *filp, char __user *user_buf,
    ...
    if (count < sizeof(led_state_char)) {
        return -EINVAL; // Validate buffer size
    }

    spin_lock(&led_lock);
    led_state_char = (led_state == 1) ? '1' : '0';
    spin_unlock(&led_lock);

    if (copy_to_user(user_buf, &led_state_char, sizeof(led_state_char))) {
        return -EFAULT; // Handle copy_to_user error
    }

    printk(KERN_INFO "Read LED state: %c\n", led_state_char);
    return sizeof(led_state_char);
}
...
static ssize_t led_write(struct file *filp, const char __user *user_buf,
    ...
    if (value == '1' || value == '0') {
        ...
        printk(KERN_INFO "Invalid input. Please enter '1' to turn on the LED or '0' to turn it off.\n");
    }
    spin_unlock(&led_lock);
