...
// Open operation
static int device_open(struct inode *inode, struct file *file) {   //device open api it will open the device 
								   //and print message in kernel space
  printk(KERN_INFO "String Device opened\n");
  return 0;
}
static int device_release(struct inode *inode, struct file *file) { //close the device and msg to kernel space
...
static int __init string_dev_init(void) {                //initialize module by string_dev_init api
	major = register_chrdev(0, DEVICE_NAME, &fops);    //register the device with mejor number
	if (major < 0) {
...
static void __exit string_dev_exit(void) {    //exit the module
