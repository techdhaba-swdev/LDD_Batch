...
static int mychardev_open(struct inode *inode, struct file *file) {   //this api called when user space program open device file
								      //and print the message 
  printk(KERN_INFO "MYCHARDEV: Device opened\n");
  return 0;
}

static int mychardev_release(struct inode *inode, struct file *file) {   //This api called when user close the device file
