...
#define DEVICE_NAME "datatransfer"
...
/*static ssize_t my_read(struct file *filp, char __user *user_buf,
    ...
    // Simulate some data (replace with your actual logic)
    data.value = 7;
    snprintf(data.message, sizeof(data.message), "Hello from kernel!");

    // Acquire spinlock to protect critical section
    spin_lock_irqsave(&my_lock, flags);

    // Copy data to user space
    if (copy_to_user(user_buf, &data, DATA_SIZE)) {
