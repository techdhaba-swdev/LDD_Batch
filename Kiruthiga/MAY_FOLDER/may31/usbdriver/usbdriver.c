...
static ssize_t usb_read(struct file *file, char __user *user_buffer, size_t count, loff_t *ppos) {
    ...
    if (retval) {
        printk(KERN_ERR "usb_driver: bulk message returned %d\n", retval);
        return retval;
    }
...
static int usb_probe(struct usb_interface *interface, const struct usb_device_id *id) {
    ...
    if (major_number < 0) {
        printk(KERN_ERR "usb_driver: failed to register a major number\n");
        return major_number;
    }
    ...
    if (IS_ERR(usb_class)) {
        ...
        printk(KERN_ERR "usb_driver: failed to register device class\n");
    ...
    if (IS_ERR(usb_device)) {
        ...
        unregister_chrdev(major_number, DEVICE_NAME);
        printk(KERN_ERR "usb_driver: failed to create the device\n");
