...
int main() {
    ...
    // Writing to the device using write system call
    ...
    //  Reading from the device using read system call
    ...
    // Writing to the mapped memory
    strcpy((char *)mapped_memory, "Hello from user space via mmap!");

    // Reading from the mapped memory
    printf("Read from device from mmap: %s\n", (char *)mapped_memory);

    // Reading the buffer data directly and printing it
    char *mapped_read_buffer = (char *)mapped_memory;
    printf("Read buffer data from mmap: %s\n", mapped_read_buffer);
