...
static ssize_t my_read(struct file *filp, char __user *user_buf,
    ...
    // Example data to read (replace with actual data source)
    data.value = 42;
    strncpy(data.message, "Hello from kernel", sizeof(data.message));

    // Acquire spinlock to protect critical section
    spin_lock_irqsave(&my_lock, flags);

    // Copy data from kernel to user space
    if (copy_to_user(user_buf, &data, min(count, DATA_SIZE))) {
...
static const struct file_operations my_fops = {
    ...
    .open = my_open,
    .release = my_release,
    .read = my_read, // Add read function
    .write = my_write,
};
