...
static int __init double_integer_init(void) {
    ...
    printk(KERN_INFO "subtract Integer: Device registered with major number %d\n", major);
    return 0;
}

static void __exit double_integer_exit(void) {
    unregister_chrdev(major, DEVICE_NAME);
    printk(KERN_INFO "subtract Integer: Device unregistered\n");
}
