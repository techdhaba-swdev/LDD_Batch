...
#define DEVICE_NAME "memoryrw_device"
...
static ssize_t my_device_read(struct file *file, char __user *buffer, size_t length, loff_t *offset) {
    if (*offset >= DEVICE_MEMORY_SIZE) return 0;
    if (*offset + length > DEVICE_MEMORY_SIZE) length = DEVICE_MEMORY_SIZE - *offset;
    if (copy_to_user(buffer, device_memory + *offset, length)) return -EFAULT;
    *offset += length;
    return length;
}
...
static ssize_t my_device_write(struct file *file, const char __user *buffer, size_t length, loff_t *offset) {
    if (*offset >= DEVICE_MEMORY_SIZE) return 0;
    if (*offset + length > DEVICE_MEMORY_SIZE) length = DEVICE_MEMORY_SIZE - *offset;
    if (copy_from_user(device_memory + *offset, buffer, length)) return -EFAULT;
    *offset += length;
    return length;
}
...
static const struct file_operations my_device_fops = {
    ...
    .read = my_device_read,  // Added read operation
    .write = my_device_write,  // Added write operation
    .mmap = my_device_mmap,
};
