...
static dev_t dev_number; //Declared a variable of type dev_t to store major number
static struct cdev cdev; //created a char dev structrue
...
// Write operation
static ssize_t reverse_dev_write(struct file *filp, const char __user *user_buf,size_t count, loff_t *f_pos) {
...
static ssize_t reverse_dev_read(struct file *filp,char * user_buff,size_t count,loff_t *f_pos){
	if(copy_to_user(user_buff,message,count)){
                return -1;
        }
        printk(KERN_INFO"Device read successfull");
        return count;
}
// Close operation
