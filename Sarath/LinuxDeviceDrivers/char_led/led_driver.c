...
static ssize_t led_read(struct file *filp,char *buf,size_t count, loff_t *off);
char value;
...
static ssize_t led_read(struct file *filp,char *buff,size_t len,loff_t *of){
    if (len != sizeof(value)) {
        return -EINVAL; // Validate data size
    }

    spin_lock(&led_lock);
    value = (value ==1)? 0: 1; // Get current LED state
    spin_unlock(&led_lock);
...
static int __init led_driver_init(void) {
    ...
    printk(KERN_INFO"MAJOR %d MINOR %d",MAJOR(led_dev),MINOR(led_dev));
