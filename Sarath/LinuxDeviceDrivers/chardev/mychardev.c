...
static struct file_operations mychardev_fops = { //declared a structure
    .owner = THIS_MODULE,
    .open = mychardev_open,
...
static int mychardev_open(struct inode *inode, struct file *file) { //opened the device using mychardev_open api
  printk(KERN_INFO "MYCHARDEV: Device opened\n");
  return 0;
}
static int mychardev_release(struct inode *inode, struct file *file) { //closing the device
  printk(KERN_INFO "MYCHARDEV: Device closed\n");
  return 0;
}

static ssize_t mychardev_read(struct file *file, char __user *user_buffer, size_t count, loff_t *offset) { //reading message into device
