...
struct simple_dma_dev { //this structure holds the info about device....has pointers to pci device,cpu memory add,dma addr,scatter gatherlist
    struct pci_dev *pdev;
    void *cpu_addr;
    dma_addr_t dma_handle;
    struct scatterlist sg; //process of gathering data from or scattering data into a set of buffer
};
//pci driver functions(peripherel component interconnect)
static int simple_dma_probe(struct pci_dev *pdev, const struct pci_device_id *ent);//called when kernel detects a pci device matching the id specified in simple_dma_ids
										   //this api allocates mem for simple-dma-dev struct,enables pci,initialize scatter gather list,maps the scatter gather list for dma (sg),copies some data to cpu side buffer
static void simple_dma_remove(struct pci_dev *pdev); //this api is called when pci is removedfrom system....performs unmaps scatter gather list,free the dma-buffer,disable pci,free mem allocated for simple_dma_dev
