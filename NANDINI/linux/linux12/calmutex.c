...
#include <linux/mutex.h> // Added mutex header
...
static ssize_t calculator_dev_write(struct file *file, const char *buf, size_t count, loff_t *ppos) {
    ...
    switch (operation) {
        ...
        case '/':
            ...
            break;
        default:
            mutex_unlock(&calculator_mutex); // Release mutex before returning error
...
static void __exit calculator_dev_exit(void) {
    ...
    mutex_destroy(&calculator_mutex); // Destroy the mutex
