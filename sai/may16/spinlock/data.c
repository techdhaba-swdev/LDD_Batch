...
static ssize_t my_read(struct file *file,char __user *user_buf, size_t count, loff_t *f_pos){
	unsigned long flags;
  ssize_t ret;
   struct user_data data;
  if (count != DATA_SIZE) {
    pr_err("Invalid data size for read: %zu, expected %zu\n", count, DATA_SIZE);
    return -EINVAL;
  }

  spin_lock_irqsave(&my_lock, flags);

  if (copy_to_user(&data, user_buf, count)) {
  ...
  ret = DATA_SIZE;
  pr_info("Sent data: value = %d, message = %s\n", data.value, data.message);
