...
static int __init reverse_dev_init(void) {
  ...
  major = register_chrdev(0,DEVICE_NAME,&reverse_dev_fops);
  if(major < 0){
  printk(KERN_ALERT "Reverse char dev failed with %d\n",major);
  return major;
  }
  printk(KERN_INFO " Reverese String: Device register with Major Number %d\n",major);
  return 0;
