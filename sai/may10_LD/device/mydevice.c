...
static ssize_t driver_read(struct file *f, char __user *user_buffer, size_t count, loff_t *offset) {
  ...
  int bytes_to_read = min(count,(size_t)sizeof(data));

  int bytes_read  ;
  mutex_lock(&driver_mutex);
  // ... read data and copy to user_buffer ...
  bytes_read = copy_to_user(user_buffer,data,bytes_to_read);
  mutex_unlock(&driver_mutex);
  if (bytes_read != 0) {
    return -EFAULT; // Error occurred during copy_to_user
  }
...
static ssize_t driver_write(struct file *f, const char __user *user_buffer, size_t count, loff_t *offset) {
  ...
  int bytes_to_write = min(count, (size_t)sizeof(data)); // Assuming 'data' is the buffer holding device data
  int bytes_written;
  mutex_lock(&driver_mutex);
  // ... copy data from user_buffer ...
  bytes_written = copy_from_user(data, user_buffer, bytes_to_write);
  mutex_unlock(&driver_mutex);
  if (bytes_written != 0) {
    return -EFAULT; // Error occurred during copy_from_user
  }
