...
static ssize_t device_read(struct file *file, char __user *buffer, size_t len, loff_t *offset) {
    ...
    // Wait until the flag is set (non-zero)
    wait_event_interruptible(my_queue, flag != 0);

    // Lock the mutex before accessing the shared resource
    mutex_lock(&my_mutex);

    // Capture the current flag value to send to user space
    int value_to_send = flag;

    // Reset the flag after capturing its value
    flag = 0;
    ...
    ret = copy_to_user(buffer, &value_to_send, sizeof(value_to_send));
    if (ret) {
        return -EFAULT;
    }

    printk(KERN_INFO "Data read from device\n");
    return sizeof(value_to_send);
}
