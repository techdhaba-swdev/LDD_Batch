...
#define DEVICE_NAME "add_10"
#define NUMBERS_COUNT 10
#define DEVICE_MEMORY_SIZE (sizeof(int) * NUMBERS_COUNT + sizeof(char))
...
static ssize_t my_device_write(struct file *file, const char __user *buffer, size_t len, loff_t *ppos) {
    ...
    memcpy(numbers, device_memory, sizeof(int) * NUMBERS_COUNT);
    memcpy(&operation, device_memory + sizeof(int) * NUMBERS_COUNT, sizeof(char));
