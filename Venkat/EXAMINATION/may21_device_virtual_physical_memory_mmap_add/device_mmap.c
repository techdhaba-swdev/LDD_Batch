...
static ssize_t my_device_write(struct file *file, const char __user *buffer, size_t len, loff_t *ppos) {
    ...
    if (copy_from_user(device_memory, buffer, len)) { // here buffer data write int device_memory like page tables 
        return -EFAULT;
    }

    memcpy(&num1, device_memory, sizeof(int)); // from here iam updating page table in linear array type 
    memcpy(&num2, device_memory + sizeof(int), sizeof(int)); // its like num1 in page/table1[0],and num2 page/table[1] in one page/table

    // Perform addition
    result = num1 + num2;

    memcpy(device_memory, &result, sizeof(int)); // result is strored in device_memeory in nextpass as an argument read from device_memeory and send it to user space
