...
#include <linux/mutex.h> // Include for the mutex
...
static int major;
static int value;
static struct mutex value_mutex; // Define a mutex
...
static ssize_t device_read(struct file *filp, char *buffer, size_t length, loff_t *offset) {
    mutex_lock(&value_mutex); // Lock the mutex before accessing the value
    copy_to_user(buffer, &value, sizeof(int));
    mutex_unlock(&value_mutex); // Unlock the mutex after accessing the value
...
static int __init double_integer_init(void) {
    ...
    mutex_init(&value_mutex); // Initialize the mutex
