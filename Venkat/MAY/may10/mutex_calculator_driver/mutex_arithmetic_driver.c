...
#define DEVICE_NAME "mutex_arithmetic"
...
static struct mutex arithmetic_mutex; // Declare a mutex
...
static ssize_t device_read(struct file *filp, char *buffer, size_t length, loff_t *offset) {
    ...
    // Ensure mutual exclusion on the result
    mutex_lock(&arithmetic_mutex);
...
static ssize_t device_write(struct file *filp, const char *buffer, size_t length, loff_t *offset) {
    ...
    // Ensure mutual exclusion on the operation
    mutex_lock(&arithmetic_mutex);
    switch (operation) {
        case '+':
...
static int __init arithmetic_operations_init(void) {
    // Initialize the mutex before registering the device
    mutex_init(&arithmetic_mutex);
...
static void __exit arithmetic_operations_exit(void) {
    // Destroy the mutex before unregistering the device
    mutex_destroy(&arithmetic_mutex);
