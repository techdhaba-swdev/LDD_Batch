...
// read operation
 static ssize_t reverse_dev_read(struct file *filp,  char __user *user_buf,
                                size_t count, loff_t *f_pos) {
	 int bytes_read = 0;
	 // check if we are at end of the message 
	 if(*f_pos >= message_len) {
		 return 0;
		}
	 // iam looking to read exact data from the kernal message(input data stored in kernal space)
	  if(count >message_len - *f_pos) {
		  count = message_len - *f_pos;
	  }

	 //copy the reverse messge to user
