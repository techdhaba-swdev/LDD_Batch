...
static ssize_t data_store(struct kobject *kobj, struct attribute *attr, const char __user *user_buf, size_t count) {
  long tmp;
  int res;
...
static struct attribute data_attr = __ATTR(data_name, 0664, data_show, data_store);

static struct attribute *default_attrs[] = {&data_attr, NULL,};

static struct attribute_group attr_group = { .attrs = default_attrs,};
