...
static int __init randomdev_init(void)
{
    ...
    if (Major < 0) {
        printk(KERN_ALERT "Registering randomnum device failed with %d\n", Major);
        return Major;
    }
    printk(KERN_INFO "Registered randomnum device with major number %d\n", Major);
    return 0;
}
...
static void __exit randomdev_exit(void)
{
    ...
    printk(KERN_INFO "Unregistered randomnum device\n");
}
